...
class Bank:
	...
	def createAccount(self):
		flag=inputf("Do you already have an Account(y/n): ")
	...
	def getTransactionReport(self):
		...
		if(choice=='1'):
			...
			if(custid not in self.Cust):
				...
				return
			l=list(filter(lambda x:((self.Acc[x.acc1].customer.custid==custid or self.Acc[x.acc2].customer.custid==custid) and date1<=x.date<=date2),self.Trans))
		elif(choice=='2'):
			branchno=int(inputf("Enter Branchno: "))
			if(branchno>=self.nextBranch):
				print("Branch Does not exist")
				return
			l=list(filter(lambda x:((self.Acc[x.acc1].branch.branchno==branchno or self.Acc[x.acc2].branch.branchno==branchno) and date1<=x.date<=date2), self.Trans))
		elif(choice=='3'):
			l=self.Trans
		self.printTrans(l)
